...
function resolveLogChannelIds(options = {}) {
  ...
  if (category === 'donut') ids.push(process.env.DONUT_PAYMENT_LOGS_CHANNEL_ID);
  if (category === 'ltc') ids.push(process.env.LTC_PAYMENT_LOGS_CHANNEL_ID);
  if (category === 'ticket') ids.push(process.env.TICKET_LOGS_CHANNEL_ID, process.env.TICKET_TRANSCRIPTS_CHANNEL_ID);
